...
app.post('/user', async (req, res) => {
  ...
  try {
    ...
    const emailCheck = await db.query('select * from user_manage where email=$1', [req.body.email])
...
app.post('/login', async (req, res) => {
  ...
  try {
    const userExists = await db.query('select * from user_manage where email=$1 or password=$2', [email, password]);
